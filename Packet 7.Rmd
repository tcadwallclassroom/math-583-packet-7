...
You will turn in the results of the fluvoxamine study above, written in such a way as to present the results to someone not in this class. That is, you should present the study with null and alternative hypotheses, the data that was collected, the statistic and p-value, and whether or not we reject the null hypothesis. You should calculate the p-value with a z-score, a second time with a randomization test, and finally, a third time using the hypergeometric exact test. In each case, the observed data is the same, we are only changing how we calculate the p-value.
